...
from modopt.math.metrics import *
...
class MetricCost(costObj):
    """ Define the cost function based on SSIM metric.
    """
    def __init__(self, linear_op, metric=ssim, initial_cost=1e6,
                 tolerance=1e-4, cost_interval=1, test_range=4, verbose=True,
                 plot_output=None):
        """ Initialize the 'MetricCost' class.
        ...
        costFunc: class
            Class for calculating the cost
        metric: function
            Function for calculating the cost (default is 'ssim')
        initial_cost: float, optional
            Initial value of the cost (default is "1e6")
        tolerance: float, optional
            Tolerance threshold for convergence (default is "1e-4")
        ...
        self.linear_op = linear_op
        self.metric = metric
        super(MetricCost, self).__init__(
            operators=None, initial_cost=initial_cost,
            tolerance=tolerance,
            cost_interval=cost_interval, test_range=test_range,
            verbose=verbose, plot_output=plot_output)
    ...
    def _calc_cost(self, x_new, y_new, *args, **kwargs):
        ...
        """
        x_dual_new = self.linear_op.adj_op(y_new)
        return self.metric(x_new, x_dual_new, None)
